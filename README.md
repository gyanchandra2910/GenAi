...
### NSE Intelligence Platform — Powered by Groq & LangChain
...
> **Team: The Silicon Savants** | ET Gen AI Hackathon
...
**AI for the Indian Investor** is a dual-agent intelligence backend that processes real-time NSE stock data and company news using large language models to surface **actionable investment signals** — the kind previously available only to institutional traders.

The platform runs two specialised AI agents in parallel for every analysis request:

| Agent | Input | Output |
|-------|-------|--------|
| 🔭 **Opportunity Radar** | Company news & filings | Fundamental signals (bulk deals, insider buys, earnings beats) |
| 📊 **Chart Intelligence** | OHLCV price history | Technical signals (RSI, SMA crossovers, Bollinger breakouts) + plain-English narrative |
...
- **🛡️ Fault-Tolerant Architecture** — Parallel `asyncio.gather` execution; if one agent fails, the other still returns results (`partial` status)
...
```bash
git clone https://github.com/your-org/ai-indian-investor.git
cd ai-indian-investor
```
  ...
  "fundamental_signals": [
    {
      ...
      "confidence": 0.82,
      "summary": "TCS signed a multi-year digital transformation deal with a European financial institution, signalling continued momentum in its BFSI vertical."
    }
  ],
  "technical_signals": [
    {
      ...
      "direction": "BULLISH",
      "confidence": 0.65,
      "narrative": "TCS is currently trading above both its 20-day (₹3,421) and 50-day (₹3,387) simple moving averages, confirming a sustained uptrend. The RSI at 58 indicates healthy momentum without being overbought. Key resistance is near the 52-week high of ₹4,592 — a breakout above this level would be strongly bullish. Investors can consider accumulating on dips towards the SMA-20 level of ₹3,421."
    }
  ],
  "data_preview": {
    "ticker": "TCS.NS",
...
Any NSE-listed equity symbol. Examples:
...
- [ ] React dashboard frontend with interactive charts
